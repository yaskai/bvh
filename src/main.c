...
int main() {
	...
	RenderTexture rt = LoadRenderTexture(1920, 1080);
	...
	while(!WindowShouldClose()) {
		...
		if(map.build_complete && time > (map.build_time + 0.9f))
		...
		if(!map.build_complete) show_build = true;		
		else if(IsKeyPressed(KEY_B)) show_build = !show_build;
		...
		DrawTextEx(font, TextFormat("build time: %.04fs", map.build_time), (Vector2) { 0, 80 }, 32, 1, LAVENDER);
		DrawTextEx(font, TextFormat("leaf hits: %d", vis_node_count), (Vector2) { 0, 120 }, 32, 1, LAVENDER);
